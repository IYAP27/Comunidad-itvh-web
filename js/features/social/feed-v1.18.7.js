...
import { htmlFeedPost } from './feed-post-renderer-v1.18.7.js';
...
function _pintarPosts() {
  ...
  `;
  _wirePosts();
  _asegurarControlesReporte();
}

// v1.18.7: respaldo visual independiente del renderer/CSS anterior.
// Si una publicación ajena no trae el control de reporte, se inyecta
// directamente en su header con estilos inline para que siempre sea visible.
function _asegurarControlesReporte() {
  const uid = usuarioActual()?.id ?? null;
  _contenedor?.querySelectorAll('.post-card').forEach((card) => {
    const post = _posts.find((p) => p.id === card.dataset.postId);
    const header = card.querySelector('.post-header');
    if (!post || !header || post.autor_id === uid) return;
    if (header.querySelector('[data-accion="reportar"]')) return;

    const btn = document.createElement('button');
    btn.type = 'button';
    btn.dataset.accion = 'reportar';
    btn.setAttribute('aria-label', 'Reportar publicación');
    btn.title = 'Reportar publicación';
    btn.textContent = '🚩 Reportar';
    btn.style.cssText = [
      'margin-left:auto', 'align-self:flex-start', 'display:inline-flex',
      'align-items:center', 'justify-content:center', 'gap:6px',
      'padding:7px 10px', 'border-radius:999px',
      'border:1px solid rgba(255,176,32,.45)',
      'background:rgba(255,176,32,.14)', 'color:#ffb020',
      'font:inherit', 'font-size:12px', 'font-weight:700',
      'line-height:1', 'cursor:pointer', 'position:static',
      'inset:auto', 'transform:none', 'z-index:2'
    ].join(';');
    btn.addEventListener('click', (ev) => {
      ev.preventDefault(); ev.stopPropagation();
      _manejarAccionPost('reportar', post, btn);
    });
